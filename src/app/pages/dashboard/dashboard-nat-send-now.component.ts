...
@Component({
  selector: 'app-dashboard-nat-send-now',
  template: `<general-data-options [uploadGuide]="upoladGuideConf" [listData]="lstOrders" [dataSearch]="dataSearch"></general-data-options>
  <general-orders-table [cols]="cols" [listOfAllData]="lstOrders" (refresh)="getData()" [typeReport]="'nacional-envialo-ya'"
  [exportPdf]="false" [editCourier]="{couriers:false}" [missings]="true"></general-orders-table>`,
  styleUrls: ['./dashboard.component.scss']
})
export class DashboardNatSendNowComponent implements OnInit, OnDestroy {
  ...
  private searchData(){
    ...
    this._dS.getOrders20JSON('NO_EXPRESS').subscribe(
    (rta:any) => {
  ...
  private initCols(){
    ...
    this.cols.splice(6, 1, {
      ...
      label: 'Ciudad',
      sortOrder: null,
      sortFn: (a: any, b: any) => String(a.storeNameCity).localeCompare(String(b.storeNameCity)),
    })
  ...
  private initConfOptions(){
    ...
    this.dataExport = {
        ...
        nameReport: 'EnvialoYaNacionalesDomicilio'
    }
    this.dataExportDetail = {
        ...
        nameReport: 'EnvialoYaNacionalesDetalleDomicilio'
    }
    this.dataSearch = {
