...
@Component({
  ...
  template: `<general-orders-table [cols]="cols" orderType="EMITTED" [listOfAllData]="lstOrders" (refresh)="getData()" ></general-orders-table>`,
  styleUrls: ['./dashboard.component.scss']
})
export class DashboardIssuedComponent implements OnInit, OnDestroy {
  ...
  private searchData(){
    ...
    this._dS.getOrders20JSON('EMITTED').subscribe(
    (rta:any) => {
      let lstOrders = rta.data.map(x =>{
        x.storeNameCity = '<u>'+ x.storeName + '</u> ' + x.city;
        x.orderDeliveryType = 'EMITTED';
