...
@Component({
  selector: 'app-dashboard-callcenter',
  template: `<general-orders-table [cols]="cols" orderType="CALLCENTER" [listOfAllData]="lstOrders" (refresh)="getData()" ></general-orders-table>`,
  styleUrls: ['./dashboard.component.scss']
})
export class DashboardCallcenterComponent implements OnInit, OnDestroy {
  ...
  private searchData(){
    ...
    this._dS.getOrders20JSON('CALLCENTER').subscribe(
    (rta:any) => {
      let lstOrders = rta.data.map(x => {
        x.storeNameCity = '<u>'+ x.storeName + '</u> ' + x.city;
        x.orderDeliveryType = 'CALLCENTER';
