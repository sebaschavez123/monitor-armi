...
@Component({
  selector: 'app-dashboard-messengers',
  template: `<general-orders-table [cols]="cols" [listOfAllData]="lstOrders" (refresh)="getData()" [typeReport]="'messengers'"
  ></general-orders-table>`,
  styleUrls: ['./dashboard.component.scss']
})
export class DashboardMessengersComponent implements OnInit, OnDestroy {
  ...
  private searchData(){
    ...
    this._dS.getOrders20JSON('MESSENGERS').subscribe((rta:any) => {
        this._dS.getOrdersMessengersAssigned().toPromise().then((rta2:any) => {
                let lstOrders = rta.data.map(x =>{
                    let totalOrderAssigned=  rta2.data.filter(orders => parseInt(orders.id) === parseInt(x.orderId));
                    x.ordersAssigned = totalOrderAssigned[0] ? totalOrderAssigned[0]['ordersAssigned'] : 0;
  ...
  private initCols(){
    ...
    this.cols.splice(8, 0, {
        name: 'ordersAssigned',
        header: 'Cantidad',
        label: 'Cant',
        sortOrder: null,
        sortFn: (a: any, b: any) => Number(a.totalOrderAssigned) - Number(b.totalOrderAssigned)
      })
  }
