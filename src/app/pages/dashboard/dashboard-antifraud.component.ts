...
@Component({
  ...
  template: `<general-orders-table [cols]="cols" orderType="FRAUD" [listOfAllData]="lstOrders" (refresh)="getData()" [typeReport]="'antifraud'" 
  [exportPdf]="false" [missings]="true"></general-orders-table>`,
  styleUrls: ['./dashboard.component.scss']
})
export class DashboardAntifraudComponent implements OnInit, OnDestroy {
  ...
  private searchData(){
    ...
    this._dS.getOrders20JSON('FRAUD').subscribe(
    (rta:any) => {
  ...
  private initCols(){
    ...
    this.cols.push({
      name: 'customerName', header: 'customerName',
      label: 'Cliente', textPos: 'rigth', sortOrder: null,
