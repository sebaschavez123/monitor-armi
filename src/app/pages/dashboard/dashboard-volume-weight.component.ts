...
@Component({
  selector: 'app-dashboard-volume-weight',
  template: `<general-orders-table [cols]="cols" orderType="WEIGHT" [listOfAllData]="lstOrders" (refresh)="getData()" ></general-orders-table>`,
  styleUrls: ['./dashboard.component.scss']
})
export class DashboardVolumeWeightComponent implements OnInit, OnDestroy {
  ...
  private searchData(){
    ...
    this._dS.getOrders20JSON('WEIGHT').subscribe(
    (rta:any) => {
      let lstOrders = rta.data.map(x =>{
        x.storeNameCity = '<u>'+ x.storeName + '</u> ' + x.city;
        x.orderDeliveryType = 'WEIGHT';
